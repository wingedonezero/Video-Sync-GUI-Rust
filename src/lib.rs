...
async fn run_job_task(job: Job, config: AppConfig, and_merge: bool) -> Result<String, String> {
    ...
    let temp_dir_name = format!("job_layout_{}", chrono::Utc::now().timestamp());
    let temp_dir = PathBuf::from(&config.temp_root).join(&temp_dir_name);
    fs::create_dir_all(&temp_dir).await.map_err(|e| e.to_string())?;

    let runner = process::CommandRunner::new(config.clone(), log_clone);

    // This helper creates a placeholder layout for testing the full pipeline
    let placeholder_layout = create_test_layout(&job, &runner, &temp_dir).await?;

    let pipeline = JobPipeline::new(config, tx);
    let result = pipeline.run_job(&job, and_merge, &placeholder_layout).await;
...
async fn create_test_layout(job: &Job, runner: &process::CommandRunner, temp_dir: &Path) -> Result<Vec<TrackSelection>, String> {
    ...
    let ref_tracks_to_extract: Vec<_> = ref_info.tracks.iter().filter(|t| t.r#type != "attachments").cloned().collect();
